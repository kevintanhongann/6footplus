<g:each var="item" in="${result}">
    <p style="padding-left: 20px">• <twitterChecker:parseLinks>${item.text}</twitterChecker:parseLinks><span class="date">(posted <script type="text/javascript">document.write(prettyDate("<g:formatDate format="yyyy-MM-dd'T'HH:mm:ss'Z'" date="${item.createdAt}"/>"));</script>)</span></p>
</g:each>
