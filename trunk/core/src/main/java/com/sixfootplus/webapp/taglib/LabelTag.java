package com.sixfootplus.webapp.taglib;
...
public class LabelTag extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		if ((message == null) || "".equals(message.trim())) {
			...
			label
					.append((requiredField) ? " <span class=\"req\">*</span>"
							: "");
			label.append((colon) ? ":" : "");
			label.append("</label>");
			...
			if (validationError) {
				...
				label.append(getMessageSource().getMessage("icon.warning",
						null, locale));
				label.append("\"");
				...
				label.append(getMessageSource().getMessage("icon.warning.img",
						null, locale));
				label.append("\" />");
			}
		}
